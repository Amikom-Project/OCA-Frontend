...
import {
  TWajibPajakOrangPribadiFormSchema,
  TWajibPajakOrangPribadiModel,
...
import { Update } from '@/api/wajib-pajak-orang-pribadi-api';

type FormEditWajibPajakOrangPribadiProps = {
  initialData?: TWajibPajakOrangPribadiModel | null;
  onEditSuccess?: () => void;
};

export default function FormEditWajibPajakOrangPribadi({
  initialData,
  onEditSuccess,
}: FormEditWajibPajakOrangPribadiProps) {
  ...
  const kode_wajib_pajak_orang_pribadi =
    initialData?.kode_wajib_pajak_orang_pribadi;
  ...
  const form = useForm<TWajibPajakOrangPribadiFormSchema>({
    ...
    defaultValues: {
      ...
      nip: initialData?.nip || '',
      ptkp: initialData?.ptkp || '',
      kewarganegaraan: initialData?.kewarganegaraan || '',
      nama_negara: initialData?.nama_negara || '',
      nik: initialData?.nik || '',
      nama_ktp: initialData?.nama_ktp || '',
      file_foto_ktp: initialData?.file_foto_ktp || '',
      no_passport: initialData?.no_passport || '',
      nama_passport: initialData?.nama_passport || '',
      masa_berlaku_passport: initialData?.masa_berlaku_passport
        ? new Date(initialData.masa_berlaku_passport)
        : undefined,
      file_foto_passport: initialData?.file_foto_passport || '',
      ...
      ada_npwp: initialData?.ada_npwp || 'Tidak',
  ...
  const onSubmit = async (data: TWajibPajakOrangPribadiFormSchema) => {
    if (!kode_wajib_pajak_orang_pribadi) {
      return;
    }

    const response = await Update(data, kode_wajib_pajak_orang_pribadi, token);
